...
def main():
    ...
    while True:
        cmd = input("请输入城市名或指令:")
        status,result = fetchWeather(cmd)
        #print (status)
        if status == 200: 
           ...
           # weather_p = u'更新时间:{},{}的天气状况是:{},温度:{}摄氏度'.format(last_update,cmd,now_text,now_temperature)

            now_text = result['results'][0]['now']['text']
            now_temperature = result['results'][0]['now']['temperature']
